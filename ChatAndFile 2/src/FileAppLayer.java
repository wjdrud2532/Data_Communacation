...
public class FileAppLayer implements BaseLayer {
    ...
    public boolean sortFileList(int lastFrameNumber) {
        ...
        if((fileByteList.size() - 1 != lastFrameNumber) || (receivedLength != targetLength)) {
            ((ChatAndFileDlg)this.GetUpperLayer(0)).ChattingArea.append("파일 수신 실패\n");
            return false;
        }
    ...
    public void setAndStartSendFile() {
        ChatAndFileDlg upperLayer = (ChatAndFileDlg) this.GetUpperLayer(0);
        ...
        try (FileInputStream fileInputStream = new FileInputStream(sendFile)) {
            ...
            ((ChatAndFileDlg)this.GetUpperLayer(0)).progressBar.setMaximum(sendTotalLength);
            if(sendTotalLength <= 1448) {
                ...
                ((ChatAndFileDlg)this.GetUpperLayer(0)).progressBar.setValue(sendedLength);
            } else {
                sendedLength = 0;
                // 파일 정보 송신
    ...
    public synchronized boolean Receive(byte[] input) { // 데이터를 수신 처리 함수
        ...
        if(checkReceiveFileInfo(input)) { // 파일의 정보를 받은 경우
            ...
            // TP: 커스텀 저장폴더 설정.
            file = new File(((ChatAndFileDlg)this.GetUpperLayer(0)).receiveFolderPath + "/" + fileName); //받는 경로..

            // TP: 리시버는 파일 용량을 미리 확인.
            ((ChatAndFileDlg)this.GetUpperLayer(0)).ChattingArea.append("수신중인 파일 용량은 " + targetLength + " 바이트 입니다.\n");

            // Progressbar 초기화
            ((ChatAndFileDlg)this.GetUpperLayer(0)).progressBar.setMinimum(0);
            ((ChatAndFileDlg)this.GetUpperLayer(0)).progressBar.setMaximum(targetLength);
            ((ChatAndFileDlg)this.GetUpperLayer(0)).progressBar.setValue(0);
            ...
            if (checkNoFragmentation(input)) {
                ...
                if(checkLastDataFrame(input)) {
                    ...
                    if(sortFileList(lastFrameNumber)) {
                        // 파일 저장 시 쓰레드로 처리하여 하면이 멈추지 않도록 함.
                        new Thread(() -> {
                            try(FileOutputStream fileOutputStream = new FileOutputStream(file)) {
                                for (int frameCount = 0; frameCount < (lastFrameNumber + 1); ++frameCount) {
                                    byte[] d = RemoveCappHeader(fileSortList.get(frameCount), fileSortList.get(frameCount).length);
                                    fileOutputStream.write(d);
                                }
                                ((ChatAndFileDlg)this.GetUpperLayer(0)).ChattingArea.append("파일 수신 및 생성 완료\n");
                                fileByteList = new ArrayList();
                ...
                ((ChatAndFileDlg)this.GetUpperLayer(0)).progressBar.setValue(receivedLength); // Progressbar 갱신
            }
        }
